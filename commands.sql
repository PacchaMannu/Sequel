...
SELECT S.NAME AS STUDENT, P.PACKAGE AS CURRENT, (P.PACKAGE + P.PACKAGE*0.1) AS INCREASED FROM STUDENT S INNER JOIN PLACEMENTS P ON P.S_ID=S.ID WHERE S.CGPA<9;

UPDATE PLACEMENTS SET PACKAGE=(PACKAGE + PACKAGE*0.1) WHERE S_ID IN (SELECT ID FROM STUDENT WHERE CGPA<9);

UPDATE PLACEMENTS P INNER JOIN STUDENT S ON S.ID=P.S_ID SET PACKAGE=(PACKAGE + PACKAGE*0.1) WHERE S.CGPA<9;

------------------------------------------------------------------------------------------------------------------------

ALTER TABLE STUDENT_01 ADD MARKS_D DECIMAL(10,4);

UPDATE STUDENT_01 SET MARKS_D=RAND()*100 WHERE 1; 

ALTER TABLE STUDENT_01 ADD MARKS_DD DECIMAL(10,4);

ALTER TABLE STUDENT_01 ADD MARKS_F FLOAT(10,4);

UPDATE STUDENT_01 SET MARKS_DD=(SELECT MARKS_D) WHERE 1;

UPDATE STUDENT_01 SET MARKS_F=(SELECT MARKS_D) WHERE 1;
